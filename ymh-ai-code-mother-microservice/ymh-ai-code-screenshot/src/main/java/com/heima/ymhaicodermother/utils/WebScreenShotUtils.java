...
public class WebScreenShotUtils {
     ...
     */
    public static String saveWebPageScreenshot(String wenUrl){
        if(StrUtil.isBlank(wenUrl)){
        ...
        try {
            ...
            String iamgeSavePath=rootPath+ File.separator+ RandomUtil.randomNumbers(5)+IMAGE_SUFFIX;
            webDriver.get(wenUrl);
            waitForPageLoad(webDriver);
            byte[] screenshotBytes=((TakesScreenshot) webDriver).getScreenshotAs(OutputType.BYTES);
            saveImage(screenshotBytes,iamgeSavePath);
            log.info("原始截图保存成功，截图路径:{}",iamgeSavePath);
            ...
            compressImage(iamgeSavePath,compressedImagePath);
            log.info("压缩图片保存成功 {}",compressedImagePath);

            FileUtil.del(iamgeSavePath);
            return compressedImagePath;
        } catch (Exception e) {
            log.error("网页截图失败：{}",wenUrl, e);
            return null;
        }
    }
